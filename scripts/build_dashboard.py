...
<header class="hero py-8 px-6">
  <div class="max-w-6xl mx-auto">
    <div class="flex items-center gap-4 mb-2">
      <span class="mono text-3xl font-bold" style="color: var(--asu-gold)">TABHS</span>
      <span class="text-sm uppercase tracking-wider" style="color: var(--asu-gold)">CIS 509, ASU W. P. Carey</span>
    </div>
    <h1 class="text-2xl font-light mb-3">Trust-Adjusted Business Health Score, Arizona restaurants</h1>
    <p class="text-sm opacity-90 max-w-3xl">
      Per-business scoring with cryptographically signed JSON receipts. Click any row to inspect
      its receipt: inputs, evidence, model lineage, dual hash. Click <span class="mono">Verify</span>
...
<main class="max-w-6xl mx-auto px-6 py-8">
  ...
  <section class="mb-8">
    ...
    <div class="flex gap-2 mb-3" id="receipt-actions" style="display: none;">
      <button onclick="verifyReceipt()" class="px-3 py-1.5 text-sm rounded mono"
              style="background: var(--asu-gold); color: var(--bg-dark);">
        Verify SHA-256
      </button>
      <span id="verify-result" class="text-sm self-center"></span>
    </div>
...
<script>
  ...
  function renderTable() {
    ...
    TABHS_DATA.top10.forEach((row, i) => {
      ...
      tr.innerHTML = `
        <td class="p-3 mono">${i + 1}</td>
        <td class="p-3 mono">${row.business_id}</td>
        <td class="p-3 text-right mono">${row.review_count}</td>
